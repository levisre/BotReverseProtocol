...
import socket, logging, struct, base64
...
def craftData(plainData):
	# Fixed RC4 key, generated by session_key.c
...
def index():
	...
	if "lyuchta.org" in request.url:
		if request.data != None:
			...
			app.logger.debug("%s -> %s | Seed_1: %08X | Seed_2: %08X | Checksum: %08X | KeyBlob: %s | Data: %s" % (request.remote_addr, request.url, seed_1, seed_2, checksum, byteArrayToHex(rsablob), byteArrayToHex(encryptedData)))
			# Dummy Padding
			badPadding = "\xBA\xAD\xF0\x0D"
			# 0x03 to elimilate SMTP Spamming
			plainData = struct.pack("<I12s", 0x3, badPadding*3)
			response = craftData(plainData)
			return response
